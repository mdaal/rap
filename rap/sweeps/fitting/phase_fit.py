...
def phase_fit(loop, env_var, Fit_Method = 'Multiple', Verbose = True, Show_Plot = True):
    ...
    loop.chisquare = obj(fit[bestfit].x, z_theta_c, f_c) / (f_c.shape[0] - 3)
    ...
    if Show_Plot:
        ...
        line = ax.plot(f,z_theta-obj_ls(fit[bestfit].x, z_theta, f),'g:', linewidth = 1, label = 'Fit ')
        ...
        for key in fit.keys():
            line = ax.plot(f,obj_ls(fit[key].x, z_theta, f),'b'+style[s], linewidth = 3, label = 'Data - Fit ' + key)
            s += 1
        ax.set_ylabel('Angle [rad]\nresiduals')
