...
from .data_management.load_JPL_sweep_data import load_JPL_sweep_data
...
from .sweep_array.pick_JPL_loop import pick_JPL_loop
...
class sweep():
    ...
    def load_JPL_sweep_data(self, sweep_file, iq_mixer=None):
        '''
        Import data generated by Nick Z.'s custom scripts for taking data at JPL
        '''
        ...
        self.sweep_data_columns_list, self.sweep_data_columns, self.Sweep_Array = \
            load_JPL_sweep_data(self.metadata, sweep_file, iq_mixer=iq_mixer)

        # only one sweep file per data set so pick that loop
        self.pick_JPL_loop()
    ...
    def pick_JPL_loop(self):
        # only one sweep file per data set so pick that loop
        pick_JPL_loop(self.metadata, self.loop, self.Sweep_Array)
